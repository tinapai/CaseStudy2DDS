...
employees_num$MaritalStatus = as.numeric(employees_num$MaritalStatus)
employees_num$OverTime = as.numeric(employees_num$OverTime)
employees_num$Attrition = as.numeric(employees_num$Attrition)
employees_num <- employees_num %>%
  select_if(is.numeric)

folds <- createFolds(employees_num$ID, k=2)
train <- employees_num[folds$Fold1,]
test <- employees_num[folds$Fold2,]

##run knn k=18 and make ROC
preds.knn <- knn(train[, names(train) != "MonthlyIncome"], test[, names(train) != "MonthlyIncome"], cl=as.factor(train$MonthlyIncome), k=10)
